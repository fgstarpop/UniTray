...
def get_fanqie(id, mode=0):
    if mode == 0:
        try:
            ...
            dataChap = html_get(f"https://fanqienovel.com/page/{id}")
            regexChap = r"itemId\":\"(?P<cid>\d+)\".*?title\":\"(?P<title>.*?)\""
            chaplist = re.finditer(regexChap, dataChap, re.S)
            list_chap = []
                    ...
                    list_chap.append(
                        {
                            ...
                            "vip": False,
                            "namechap": chap["title"],
                            "linkchap": f"https://fanqienovel.com/reader/{chap['cid']}",
                        }
                    )
...
if __name__ == "__main__":
    uvicorn.run("a:app", host="0.0.0.0", port=8000, server_header=False)
