...
use App\Domain\Category\Models\CategoryParents;
...
class StoryController extends Controller
{
    public function show($story)
    {
        ...
        if ($story && empty($story->idhost)) {
            $datahost = Http::get("http://103.146.22.150:8000/gethost?link=$story->origin")->json();
...
//      Nhung
        if ($url) {
            ...
            $datahost = Http::timeout(10)->get("http://103.146.22.150:8000/gethost?link=$url")->json();
            if (isset($datahost) && isset($datahost['bookid']) && isset($datahost['host'])) {
...
//            }
    ...
    public function filters(Request $request)
    {
        ...
        if ($category) {
            $idsArray = Category::where("parents_categor_id", $category)->pluck('id')->toArray();
            //dd($idsArray);
            $stories = $stories->whereHas('categories', function ($q) use ($idsArray) {
                return $q->whereIn('parents_categor_id', $idsArray);
